 ...
 *   2. engineMode "auto" — a mid-program `capability_unsupported` (hover is
 *      not implemented by the engine in M1) migrates the page to Chromium,
  ...
  itIfChromium("auto: a mid-program capability_unsupported falls back to Chromium and records the origin", async () => {
    ...
    const res = await invoke<ProgramResult>("pages.execute", {
      program: {
        ...
        steps: [
          { id: "s1", op: "select", target: "css:#status", value: "approved" },
          { id: "s2", op: "hover", target: "css:#apply-filter" }, // not implemented by the engine in M1
