...
mod tests {
    ...
    fn counts_supported_unknown_deferred_and_invalid() {
        let sheet = parse_stylesheet(
            r"
            p { color: red; margin: 1px; filter: blur(1px); frobnicate: 1; }
        ...
        assert_eq!(c.declarations_deferred, 1, "filter");
        ...
        let clean = parse_stylesheet("p { color: red; filter: none }", Origin::Author);
