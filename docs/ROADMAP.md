...
| H1-C1 | `protocolVersion`, `agent.capabilities`, token budget + ranking + cursor, `frameChain`/`shadowDepth`/`scrollContainer`/`occludedBy`, `ref_stale`, closed `VectorErrorCode` | C | ☑ | `protocolVersion: 1` on observe list; `held_out_task_controls_are_in_the_top_forty` (3/3); live `taskControlRank` in `corpus-500-latest.json` |
