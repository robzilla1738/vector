...
| H1-B5 | Cache revalidation; async resolver; `preconnect`/`prefetch`; non-blocking subresource fetch | B | ☑ | `stale_entries_are_revalidated_and_a_304_refreshes_them`; `NetworkContext::preconnect` DNS-only (`preconnect_checks_policy_without_a_get`); `<link rel=preconnect\|dns-prefetch>` + `rel=prefetch\|modulepreload` as Parser GET (`preconnect_and_prefetch_are_issued`); speculative `Initiator::Prefetch` stays denied; `HyperTransport::send_many` concurrent |
