...
| H0-B3 | Real `crypto.getRandomValues`, cycle-aware `structuredClone`, `performance.mark/measure` | B | ☑ | `__ve.randomBytes` CSPRNG (`/dev/urandom`); DOM `structuredClone` walks cycles/`Date`/`Map`/`Set` (not `JSON.stringify`); `performance.mark`/`measure`; `structured_clone_is_cycle_aware_and_crypto_is_csprng` |
...
| H1-A7 | Spec pointer/mouse sequence, hover, capture, selection, composition, `contextmenu`; human/agent events-log identical | A | ☑ | `fixtures/events-log`; `click_at_element` fires pointerdown/mousedown/pointerup/mouseup/click (right-click `contextmenu`); hover fires pointer/mouse over+enter; `Element.setPointerCapture` retargets later pointer events (`pointer_capture_retargets_pointerup`); `fill` select-all + `select` (`fill_selects_the_control_and_fires_select`); IME `compose_text` / `dispatch_human_ime` fires compositionstart/update/end (`compose_text_fires_composition_sequence`); `events_log_human_and_agent_clicks_are_byte_identical` |
| H1-B1 | Phase-0 bindings memo from dombench; `VECTOR_DOM_BINDINGS=prelude\|native` | B | ☑ | `dom_bindings_default_is_prelude`; `writes_dombench_phase0_memo`; `docs/engine/evidence/dombench-latest.json` (`todoMvcIterations: 100`, `nodesSize`/`rssBytes` filled); `VECTOR_DOM_BINDINGS=native` installs V8 FunctionTemplates for `Element.id` / `className` / `tagName` / `innerHTML` / `outerHTML` / `matches` / `getAttribute` / `setAttribute` / `removeAttribute` / `hasAttribute` / `toggleAttribute` and `Node.textContent` / `nodeType` / `nodeName` / `nodeValue` / `isConnected` / `contains` / `hasChildNodes` / `isEqualNode` / `compareDocumentPosition` (`install_native_dom_bindings`, `native_bindings_install_element_id_accessor`; missing `getAttribute` returns `null`); default stays prelude until H3-2 |
