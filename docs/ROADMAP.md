...
| H2-A2 | `crates/ve-chrome` retained widgets; tokens + `workspace.ts` + `intent.ts` port | A | ☑ | Top-pill command bar + Arc sidebar + inset stage + rail matching `docs/ui/screenshots/01-start-page.png` / `03-browsing-sidebar.png`; `docs/ui/screenshots/ve-shell-gui.png`; `docs/ui/screenshots/ve-shell-start.png` (greeting, date, hero, prompts, favourites, recents); `docs/ui/screenshots/ve-shell-palette.png`; `docs/ui/screenshots/ve-shell-settings.png`; `paints_sidebar_stage_rail_and_text` hits toolbar `CommandBar`; static Inter-Regular (`prefer_static_inter`); `system_fonts_paint_inter_ui_text`; `ve-chrome-regions.json`; headed `live_os_window_and_mcp_share_one_page` |
