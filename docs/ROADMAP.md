...
| H1-B1 | Phase-0 bindings memo from dombench; `VECTOR_DOM_BINDINGS=prelude\|native` | B | ☑ | `dom_bindings_default_is_prelude`; `writes_dombench_phase0_memo`; `docs/engine/evidence/dombench-latest.json` (`todoMvcIterations: 100`, `nodesSize`/`rssBytes` filled); `VECTOR_DOM_BINDINGS=native` installs V8 FunctionTemplates for `Element.id` / `className` / `tagName` / `innerHTML` / `outerHTML` / `matches` / `getAttribute` / `setAttribute` / `removeAttribute` / `hasAttribute` / `toggleAttribute` and `Node.textContent` / `nodeType` / `nodeName` / `nodeValue` / `isConnected` (`install_native_dom_bindings`, `native_bindings_install_element_id_accessor`; missing `getAttribute` returns `null`); default stays prelude until H3-2 |
