...
| H2-A2 | `crates/ve-chrome` retained widgets; tokens + `workspace.ts` + `intent.ts` port | A | ☑ | `docs/ui/screenshots/ve-shell-gui.png`; `docs/ui/screenshots/ve-shell-start.png` (Electron start page: greeting, hero field, prompts, pins/recents); `docs/ui/screenshots/ve-shell-palette.png` (⌘K Agent/Window command list); `docs/ui/screenshots/ve-shell-settings.png` (Appearance/Engine/Models drawer); `palette_lists_electron_commands`; `palette_command_new_tab_and_screenshot`; `settings_drawer_lists_electron_sections`; `history_overlay_hits_rows`; `start_page_paints_greeting_and_prompts`; `start_page_is_composited_instead_of_blank_document`; `system_fonts_paint_inter_ui_text`; `ve-chrome-regions.json`; Engine/Chromium badge + `route_reason`; `gui_chrome_typing_scroll_and_screenshot`; `chromium_tab_does_not_expose_engine_dom`; headed `ve-shell --features window,v8` `live_os_window_and_mcp_share_one_page` |
...
- Horizon 2 exit: MVP M1 gate (a day of ordinary browsing with restart and state retained) — `day_of_browsing_restores_tabs_history_bookmarks_zoom_find` / `docs/engine/evidence/day-of-browsing.json`; lying-model test 0 completed (`lying model cannot complete a run`); kill-9 write test exactly one POST; held-out suite with live model and competitor rows published (`docs/engine/evidence/held-out-latest.json`, `skippedLive` until API keys).
