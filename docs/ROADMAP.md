...
| H2-A2 | `crates/ve-chrome` retained widgets; tokens + `workspace.ts` + `intent.ts` port | A | ☑ | Electron default `railOpen: false` + AGENT `recent_runs`; compact `command_pill_rect`; space-dot + pins + tab letters; 📁 Dev folder (`folders_in_space`/`unfiled_tabs`); start page `PINNED` + 2-col `RECENT` + 2-col `TRY ASKING` matching 01/08 (`start_page_paints_greeting_and_prompts`); collapsed 56px rail of host-letter tiles + ⌘S (`collapsed_rail_paints_tab_tiles_not_a_lone_v`, `collapsed_rail_screenshot_matches_electron_02`); hover peek of full sidebar (`peek_paints_full_sidebar_over_rail`, `ve-shell-peek.png`); ⌘L command-bar typing + intent chip (`command_bar_shows_typed_query_and_intent_chip`, `command_bar_typing_screenshot_matches_electron_11`); `docs/ui/screenshots/ve-shell-start.png`; `docs/ui/screenshots/ve-shell-start-light.png`; `docs/ui/screenshots/ve-shell-browse.png`; `docs/ui/screenshots/ve-shell-rail.png`; `docs/ui/screenshots/ve-shell-peek.png`; `docs/ui/screenshots/ve-shell-command.png`; `ve-shell-gui.png` / palette / settings; headed `live_os_window_and_mcp_share_one_page` |
