...
| H2-A2 | `crates/ve-chrome` retained widgets; tokens + `workspace.ts` + `intent.ts` port | A | ☑ | Electron default `railOpen: false` + AGENT `recent_runs`; compact `command_pill_rect`; space-dot + pins + tab letters; 📁 Dev folder via `folders_in_space`/`tabs_in_folder`/`unfiled_tabs` (`sidebar_paints_dev_folder_then_unfiled_tabs`); start page `PINNED`/`RECENT` (`seed_design_reference`); `docs/ui/screenshots/ve-shell-start.png`; `docs/ui/screenshots/ve-shell-start-light.png` (`light_start_page_screenshot_matches_electron_08`); `docs/ui/screenshots/ve-shell-browse.png` (`browsing_sidebar_screenshot_matches_electron_03`); `ve-shell-gui.png` / palette / settings; `start_page_paints_greeting_and_prompts`; headed `live_os_window_and_mcp_share_one_page` |
