...
| H3-3 | Remaining CSS by corpus frequency; compositor animations; SVG; canvas 2D | B | ☑ | `aspect_ratio_sizes_auto_axis`; `from_layout_emits_object_position`; `from_layout_emits_text_underline`; `from_layout_emits_background_size_cover`; `from_layout_emits_outline_and_text_shadow`; `css_animation_interpolates_opacity_from_keyframes`; `Compositor::animate_opacity_at`; `from_layout_emits_box_shadow`; `from_layout_emits_linear_gradient_and_filter_blur`; `background-image: url()`/`linear-gradient`; `filter: blur()`; `transform-origin`; SVG rect+circle+ellipse+line; canvas stroke/fill/text/image |
