...
| H2-A2 | `crates/ve-chrome` retained widgets; tokens + `workspace.ts` + `intent.ts` port | A | ☑ | Electron default `railOpen: false` + AGENT footer with `recent_runs` (01/03); compact `command_pill_rect` + nav/action icons; space-dot + 5-up pin tiles + tab favicon letters + `+ New Tab`; start page greeting/date/hero/sparkle prompts/`PINNED`/`RECENT` (`seed_design_reference`); `docs/ui/screenshots/ve-shell-start.png` (1440×900); `docs/ui/screenshots/ve-shell-browse.png` (`browsing_sidebar_screenshot_matches_electron_03`); `docs/ui/screenshots/ve-shell-gui.png`; `docs/ui/screenshots/ve-shell-palette.png`; `docs/ui/screenshots/ve-shell-settings.png`; `start_page_paints_greeting_and_prompts`; `paints_sidebar_stage_rail_and_text`; static Inter-Regular; `ve-chrome-regions.json`; headed `live_os_window_and_mcp_share_one_page` |
