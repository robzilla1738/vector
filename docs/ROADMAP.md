...
| H2-A2 | `crates/ve-chrome` retained widgets; tokens + `workspace.ts` + `intent.ts` port | A | ☑ | Electron default `railOpen: false` + AGENT footer in the sidebar; compact `command_pill_rect` + nav/action icons matching `docs/ui/screenshots/01-start-page.png` / `03-browsing-sidebar.png`; space-dot + 5-up pin tiles + tab favicon letters + `+ New Tab`; start page greeting/date/hero/sparkle prompts/`PINNED` tiles/`RECENT` rows (`seed_design_reference`); `docs/ui/screenshots/ve-shell-gui.png`; `docs/ui/screenshots/ve-shell-start.png`; `docs/ui/screenshots/ve-shell-palette.png`; `docs/ui/screenshots/ve-shell-settings.png`; `start_page_paints_greeting_and_prompts`; `product_chrome_paints_sidebar_stage_and_rail`; `paints_sidebar_stage_rail_and_text` hits toolbar `CommandBar`; static Inter-Regular (`prefer_static_inter`); `ve-chrome-regions.json`; headed `live_os_window_and_mcp_share_one_page` |
