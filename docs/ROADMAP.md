...
| H0-C1 | `format` on `ObservationRequestSchema`; unify `documentEpoch`; structured `delta`; generational-ref parser | C | ☑ | service `observe(params)`; no wire `generation`; MCP `vector_page_observe` `format: full` (`tests/unit/mcp-stdio.test.ts` `reaches full observation over MCP`) |
