...
| H1-A7 | Spec pointer/mouse sequence, hover, capture, selection, composition, `contextmenu`; human/agent events-log identical | A | ☑ | `fixtures/events-log`; `click_at_element` fires pointerdown/mousedown/pointerup/mouseup/click (right-click `contextmenu`); hover fires pointer/mouse over+enter; `events_log_human_and_agent_clicks_are_byte_identical` |
