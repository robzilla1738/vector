...
| H1-B1 | Phase-0 bindings memo from dombench; `VECTOR_DOM_BINDINGS=prelude\|native` | B | ☑ | `dom_bindings_default_is_prelude`; `writes_dombench_phase0_memo`; `docs/engine/evidence/dombench-latest.json` (`todoMvcIterations: 100`, `nodesSize`/`rssBytes` filled) |
...
| H2-A4 | `ve-profile` (SQLite): history, bookmarks, session restore, downloads, find, zoom, cert interstitial, permission sheets | A | ☑ | `session_restore_reopens_tabs_after_restart`; `day_of_browsing_restores_tabs_history_bookmarks_zoom_find` (open/bookmark/download/zoom/find/present/restart step times); `docs/engine/evidence/day-of-browsing.json`; `find_bar_shows_match_count`; `cert_and_permission_sheets_persist_to_profile`; `Profile::open` rusqlite |
