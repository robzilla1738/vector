...
|---|---|---|
| `capability_unsupported` on ≥ 500 real pages | < 1% | engine 0/488 opened live HTML bodies (`engineCapabilityUnsupportedRate` 0.0); live fetch 488/500 (12 bot-wall/timeout, 2.4%) in `corpus-500-latest.json` — fetch fail is not engine `capability_unsupported`; n=488 opened, not 500; gate unmet; not a deletion license |
