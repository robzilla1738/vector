...
subset 112 PASS / 0 FAIL; official `html/dom` 272 PASS / 59 FAIL at WPT
`7c20438…`. `describe()`: `serviceWorkers:true`, `webgl:false`,
`webgpu:false`. VEC-008/010 remain subset; ve-vm is research (V8 is
production). Still-open `html/dom` FAILs: tentative partial-updates, leftover
`lang`/`dir=auto`, `document.lastModified`, `idlharness.https.html`,
`usvstring-reflection`, leftover render-blocking.
