...
export { DEFAULT_MAX_STEPS, DEFAULT_MAX_MODEL_CALLS, TERMINAL_STATUSES };
...
export async function runCoordinatorLoop(self: CoordinatorLoopHost, runId: string): Promise<void> {
    ...
    const modelId = run.config?.modelId ?? self.deps.defaultModel();
    ...
    const tryFinalDecision = async (reason: string, obs: Observation): Promise<"done" | "input" | "gave_up"> => {
      try {
        ...
        if (d.status === "needs_input" && d.question) {
          self.setStatus(runId, "needs_input", d.question);
          const answer = await self.waitForAnswer(c, runId);
        ...
        const verifyObs = await self.deps.pages.observe(activePageId, { sinceRevision: obs.revision, format: "full" });
        const verified = verifyDoneAgainstObservation(result, verifyObs, outcomes);
        if (!verified.ok) return "gave_up";
        self.finish(runId, "completed", result, undefined, d.message || "Done");
        return "done";
      } catch {
        return "gave_up";
      }
      ...
      while (true) {
        ...
        try {
          ...
          if (carriedObs && carriedObs.pageId === activePageId) {
            ...
            carriedObs = undefined;
            obs = await self.deps.pages.observe(activePageId, obsReq);
          }
        } catch (e) {
          ...
          if (e instanceof VectorError && e.code === "target_detached") {
            const next = run.pageIds.find((p) => {
              if (p === activePageId || !self.deps.pages.livePageIds().includes(p)) return false;
              const vs = self.deps.pages.get(p).viewStatus;
              return vs !== "detached" && vs !== "crashed";
            });
            if (!next) throw e;
            activePageId = next;
            self.setStatus(runId, "planning", `Page detached — moved to ${next}`);
            obs = await self.deps.pages.observe(next, obsReq);
          } else {
            throw e;
          }
        }
        ...
        const onStepRecorded = (outcome: StepOutcome, step: Step) => {
          ...
          self.deps.onStepRecorded?.(runId, outcome, step, obsArtifactId);
        ...
        const plannerCall = async (): Promise<PlanChunk> => {
          ...
          const dispatcher = new EarlyDispatcher(
            (steps, { first, last }) => {
              const live = self.deps.pages.get(activePageId!);
              ...
              const write = beginConsequentialWrite(self.durable, {
                runId,
                pageId: activePageId!,
                documentEpoch: epoch,
              ...
              return self.deps.pages
                ...
                .then((r) => {
                  settleWrite(self.durable, write.intentId, r.status === "completed");
                  return r;
                });
            },
            24,
        ...
        const reuse = tryReuseSkill(self.skills, run.goal, obs.content, obs.content.url, obs.documentEpoch);
        try {
          if ("skill" in reuse && !lastError && !visionPending && !thinObs) {
            ...
            visionUsed = true;
            visionPending = false;
            const visionId = self.deps.visionModel?.();
            if (visionId) {
              self.setStatus(runId, "planning", "Looking at the page…");
              const v = await self.tryVisionPlan({
                runId,
                pageId: activePageId,
                goal: run.goal,
                url: obs.content.url,
        ...
        if (plan.status === "done") {
          ...
          let verifyObs = obs;
          try {
            verifyObs = await self.deps.pages.observe(activePageId, { sinceRevision: obs.revision, format: "full" });
          } catch {
            verifyObs = obs;
          }
          ...
          if (!verified.ok) {
            self.finish(runId, "failed", plan.result, `done not verified: ${verified.reason}`, "Done was not grounded in the page");
            return;
          }
          self.finish(runId, "completed", plan.result, undefined, plan.message || "Done");
          return;
        }
        if (plan.status === "needs_input") {
          machine = reduce(machine, { type: "plan", status: "needs_input" });
          self.setStatus(runId, "needs_input", plan.question ?? "Needs input");
          const answer = await self.waitForAnswer(c, runId);
          if (c.abort.signal.aborted) return;
          outcomes.push({
            stepId: `input-${Date.now()}`,
        ...
        self.setStatus(runId, "running", plan.message);
        ...
        const result = write.skip
          ...
          : await self.deps.pages.execute(program, { runId, signal: c.abort.signal, onStep: onStepRecorded }, { returnObservation: nextObserveReq() });
        if (early && !streamed) early.halt();
        settleWrite(self.durable, write.intentId, result.status === "completed");
        ...
        self.persistCheckpoint(self.get(runId));
        if ("skill" in reuse && result.status !== "failed" && result.status !== "cancelled") {
          const after = carriedObs ?? (await self.deps.pages.observe(activePageId!, {}));
        ...
        if (result.status !== "failed" && result.status !== "cancelled" && plan.steps?.length && !("skill" in reuse)) {
          try {
            ...
            if (self.skills.length < 32 && named.length > 0 && !writes) {
              self.skills.push(
                compileSkill({
                  id: `${runId}:${self.skills.length}`,
          ...
          if (repairCount === MAX_REPAIRS && recoveryModelId && recoveryModelId !== modelId) {
            ...
            if (repair.object.steps.length) {
              ...
              if (!("rejected" in compiled) && !("denied" in compiled)) {
                ...
                if (!write.skip) {
                  const r2 = await self.deps.pages.execute(compiled.program, { runId, signal: c.abort.signal });
                  settleWrite(self.durable, write.intentId, r2.status === "completed");
